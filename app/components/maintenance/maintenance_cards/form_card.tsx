...
interface Card extends Form {
    ...
    prompt_two: string;
    prompt_three: string;
    handleDeleteForm: (FormsId: string) => Promise<void>;
    handleModifyForm: (form: Form) => Promise<void>;
}

const FormCard: React.FC<Card> = (promp: Card) => {
    const onDeleteClick = async () => {
        await promp.handleDeleteForm(promp.id);
      };
      const onModifyClick = async () => {
        await promp.handleModifyForm(promp);
      };
    return (
